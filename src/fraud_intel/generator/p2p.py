"""P2P channel synthetic generator (guide section 7). Deterministic for a
...
Typology: mule fan-out to a new recipient (fraud scenario) vs. a
legitimate large transfer to a new recipient (hard false positive).
"""
...
CHANNEL = "p2p"
FRAUD_SCENARIO_TYPE = "P2P_MULE_FAN_OUT"
HARD_FP_SCENARIO_TYPE = "P2P_LEGITIMATE_NEW_RECIPIENT_LARGE"

NETWORKS = ["Zelle-like", "Venmo-like", "CashApp-like"]


def generate_p2p_events(
    *,
    seed: int,
    n: int,
    reference_date: date,
    ...
    for draw in draws:
        ...
        base_amount = int(rng.integers(1_000, 20_000))
        amount_minor_units = base_amount * int(rng.integers(2, 8)) if (is_fraud or is_hard_fp) else base_amount

        payload = P2PPayload(
            recipient_handle=f"@recipient{int(rng.integers(100000, 999999))}",
            network=str(rng.choice(NETWORKS)),
            memo_present_flag=not is_fraud,
            recipient_is_new_flag=is_fraud or is_hard_fp,
        )
        event = FraudEvent(
            event_id=event_id,
            channel=CHANNEL,
