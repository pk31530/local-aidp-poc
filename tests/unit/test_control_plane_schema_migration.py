...
def test_migration_file_exists_with_next_unused_number():
    ...
    # synthetic_event_labels/channel_model_bundles) and Phase 6 adds
    # migration 004 (fraud_alerts/alert_evidence/analyst_dispositions/
    # label_assessments) -- this file otherwise covers migration 002
    # specifically and is unaffected by either addition.
...
def test_pipeline_name_allowed_values_match_between_schema_and_migration():
    # Migration 002 only ever declared the original three pipeline names --
    # v1.3 Phase 6's migration 004 (not this file's migration) is the one
    # that widens pipeline_runs_pipeline_name_check to the four additional
    # fraud-intel pipeline names now also present in schema.sql (see
    # tests/unit/test_fraud_intel_migration_004.py for that consistency
    # check). This test's job is unchanged: migration 002 in isolation
    # must still only ever have declared its own original three values.
    ...
    assert migration_values == {"batch", "train", "stream"}
    assert migration_values <= schema_values
